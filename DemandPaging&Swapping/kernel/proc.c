...
found:
  ...
  // Initialize demand paging / swapping related fields
  p->swapfile = 0;
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Propagate lazy exec metadata to child for correct fault handling
...
kexit(int status)
{
  ...
  // Cleanup per-process swap file before taking p->lock/wait_lock.
  if(p->swapfile){
    int freed_slots = 0;
